"""
H&M R25: R19 Training + R21's Key Features

R19 failed (0.01769) because it lacked item_recent_week_buys and rich features.
R21 succeeded (0.02365) with those features but used single-fold training.

R25 = R19's training (3-fold, negative sampling 1:4) + R21's features:
  - item_recent_week_buys (THE key missing feature)
  - item_sales_trend, age match, price match, freshness
"""
import pandas as pd
import numpy as np
...
print("=" * 60)
print("R25: R19 Training + R21 Features (neg sampling + recent week buys)")
print("=" * 60)
...
# Global age-demographic features (don't change per fold)
print("  Global age/price features...")
txn_with_age = txn.merge(customers[['customer_id', 'age']], on='customer_id', how='left')
global_item_user_mean_age = txn_with_age.groupby('article_id')['age'].mean().to_dict()
global_item_user_age_std = txn_with_age.groupby('article_id')['age'].std().fillna(0).to_dict()
del txn_with_age
gc.collect()
global_user_mean_price = txn.groupby('customer_id')['price'].mean().to_dict()
global_item_mean_price = txn.groupby('article_id')['price'].mean().to_dict()
global_item_freshness = txn.groupby('article_id')['days_ago'].max().to_dict()
global_user_mean_channel = txn.groupby('customer_id')['sales_channel_id'].mean().to_dict()
global_item_mean_channel = txn.groupby('article_id')['sales_channel_id'].mean().to_dict()
...
def generate_candidates_and_features(txn_df, target_week, for_training=True):
    ...
    # CRITICAL: Recent week buys (first week after target)
    first_week = target_week + 1
    feat_item_recent_week = feature_txn[feature_txn['week'] == first_week]['article_id'].value_counts().to_dict()
    feat_item_last2w = feature_txn[feature_txn['week'] <= first_week + 1]['article_id'].value_counts().to_dict()
    feat_item_last4w = feature_txn[feature_txn['week'] <= first_week + 3]['article_id'].value_counts().to_dict()
    feat_n_weeks = max(len(feature_txn['week'].unique()), 1)
    feat_item_weekly_avg = {aid: count / feat_n_weeks for aid, count in feat_item_buys.items()}
    feat_item_sales_trend = {aid: feat_item_recent_week.get(aid, 0) / max(feat_item_weekly_avg.get(aid, 0.1), 0.1) for aid in feat_item_buys}
    ...
    return {
        ...
        'feat_item_recent_week': feat_item_recent_week,
        'feat_item_last2w': feat_item_last2w,
        'feat_item_last4w': feat_item_last4w,
        'feat_item_sales_trend': feat_item_sales_trend,
...
def build_feature_row(cid, aid, ctx):
    ...
    # R21 key features: recent week buys & sales trend
    row['item_recent_week_buys'] = ctx['feat_item_recent_week'].get(aid, 0)
    row['item_last2w_buys'] = ctx['feat_item_last2w'].get(aid, 0)
    row['item_last4w_buys'] = ctx['feat_item_last4w'].get(aid, 0)
    row['item_sales_trend'] = ctx['feat_item_sales_trend'].get(aid, 0)

    # Age demographic features
    row['item_user_mean_age'] = global_item_user_mean_age.get(aid, 35)
    row['user_item_age_diff'] = abs(row['user_age'] - row['item_user_mean_age'])
    row['item_user_age_std'] = global_item_user_age_std.get(aid, 0)

    # Price features
    row['user_mean_price'] = global_user_mean_price.get(cid, 0)
    row['item_mean_price'] = global_item_mean_price.get(aid, 0)
    row['user_item_price_diff'] = abs(row['user_mean_price'] - row['item_mean_price'])

    # Freshness & channel
    row['item_freshness'] = 100 - global_item_freshness.get(aid, 100)  # higher = more recent
    row['user_mean_channel'] = global_user_mean_channel.get(cid, 1.5)
    row['item_mean_channel'] = global_item_mean_channel.get(aid, 1.5)
...
FEATURE_COLS = [
    ...
    'item_freshness', 'user_mean_channel', 'item_mean_channel',
]
...
for cid in sample_sub['customer_id']:
    ...
    for aid in ctx_pred['cust_repurchase'].get(cid, [])[:20]:
        candidates.add(aid)
...
sub_path = OUTPUTS / "submission_r25_negsam_with_recent.csv"
...
print("R25 Complete")
print(f"Val MAP@12 (active): {map12_active:.6f}")
print(f"R21: 0.02365 | R19: 0.01769 | Target: 0.0300")
print("=" * 60)
