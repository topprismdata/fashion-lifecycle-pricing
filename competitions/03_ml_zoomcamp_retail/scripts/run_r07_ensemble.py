#!/usr/bin/env python3
"""
R07 Multi-Model Ensemble for ML Zoomcamp 2024.

Based on R06 dense cross-join grid (CV 8.08, LB 15.42).
Key changes:
  1. CatBoost + LightGBM + XGBoost ensemble with optimized weights
  2. More iterations (3000, lr=0.03) for better learning on dense grid
  3. DuckDB type fix — item_id kept as string (hex hash)
  4. store_id converted to string for consistency

Target: LB < 10 (current: 15.42)

Usage:
    python scripts/run_r07_ensemble.py
"""
...
SUBMISSION_PATH = OUTPUT_DIR / "submission_r07_ensemble.csv"
...
def build_dense_grid() -> pd.DataFrame:
    ...
    # Normalize types: DuckDB returns VARCHAR.
    # item_id is hex string (e.g., "c578da8e8841") — keep as string.
    # store_id is numeric string ("1","2","3","4") — keep as string for consistency.
    # All external data store_id will be converted to string in load functions.
...
def main() -> float:
    ...
    # --- Model 1: CatBoost ---
    log("=" * 50)
    log("Model 1: CatBoost")
    log("=" * 50)
    cb_model = train_catboost(X_train, y_train, X_val, y_val, CAT_FEATURES)
    show_feature_importance(cb_model, feature_cols)

    val_pool = Pool(X_val, cat_features=CAT_FEATURES)
    val_preds_cb = cb_model.predict(val_pool)
    cb_val_rmse = np.sqrt(np.mean((val_preds_cb - y_val) ** 2))
    log(f"  CatBoost val RMSE: {cb_val_rmse:.4f} (best_iter={cb_model.get_best_iteration()})")

    test_pool = Pool(X_test, cat_features=CAT_FEATURES)
    test_preds_cb = cb_model.predict(test_pool)
    val_preds_list.append(val_preds_cb)
    test_preds_list.append(test_preds_cb)
    model_names.append("CatBoost")

    # --- Model 2: LightGBM ---
    log("=" * 50)
    log("Model 2: LightGBM")
    log("=" * 50)
    # Prepare data for LightGBM — convert categoricals to category dtype
    ...
    lgb_val_rmse = np.sqrt(np.mean((val_preds_lgb - y_val) ** 2))
    log(f"  LightGBM val RMSE: {lgb_val_rmse:.4f} (best_iter={lgb_model.best_iteration_})")
    ...
    # --- Model 3: XGBoost (SKIPPED — too slow on 7.8M rows) ---
    log("=" * 50)
    log("Model 3: XGBoost (SKIPPED)")
    log("  XGBoost is too slow on 7.8M rows with categorical features")
    log("=" * 50)
    xgb_val_rmse = None

    # --- Ensemble Weight Optimization ---
    log("=" * 50)
    log("Ensemble Weight Optimization (CatBoost + LightGBM)")
    log("=" * 50)
    ...
    def ensemble_rmse(weights, preds_list, y_true):
        ...
        return np.sqrt(np.mean((ensemble - y_true) ** 2))

    best_rmse = float('inf')
    best_weights = np.ones(len(model_names)) / len(model_names)
    for w0 in [[1, 0], [0, 1], [1, 1], [2, 1], [1, 2], [3, 1], [1, 3]]:
        res = minimize(ensemble_rmse, w0, args=(val_preds_list, y_val), method='Nelder-Mead')
        if res.fun < best_rmse:
            best_rmse = res.fun
            w = np.abs(res.x)
            best_weights = w / w.sum()

    log(f"  Optimal weights: {dict(zip(model_names, [f'{w:.3f}' for w in best_weights]))}")
    log(f"  Ensemble val RMSE: {best_rmse:.4f}")
    log(f"  Individual RMSEs: CB={cb_val_rmse:.4f}, LGB={lgb_val_rmse:.4f}")

    # Generate ensemble predictions
    final_val_preds = sum(w * p for w, p in zip(best_weights, val_preds_list))
    final_test_preds = sum(w * p for w, p in zip(best_weights, test_preds_list))

    # ================================================================
    # Post-process
    # ================================================================
    preds = postprocess_predictions(final_test_preds, dense_df, test, catalog)
    ...
    log("=" * 70)
    log("R07 Multi-Model Ensemble Summary")
    log("=" * 70)
    log(f"Ensemble val RMSE: {best_rmse:.4f}")
    log(f"  CatBoost RMSE:   {cb_val_rmse:.4f} (best_iter={cb_model.get_best_iteration()})")
    log(f"  LightGBM RMSE:   {lgb_val_rmse:.4f} (best_iter={lgb_model.best_iteration_})")
    log(f"  XGBoost: SKIPPED (too slow)")
    log(f"  Optimal weights: {dict(zip(model_names, [f'{w:.3f}' for w in best_weights]))}")
