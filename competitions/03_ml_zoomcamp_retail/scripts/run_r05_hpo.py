#!/usr/bin/env python3
"""
R05 Hyperparameter Optimization: CatBoost HPO on R03's 50 features.

Strategy: Manual grid search over 4 CatBoost configs (no Optuna dependency).
Uses the exact same feature pipeline as R03 (50 features, LB 17.17).

Configs to try:
  A: depth=10, lr=0.05, iter=2000, l2=3
  B: depth=8,  lr=0.03, iter=3000, l2=5
  C: depth=9,  lr=0.05, iter=2000, l2=5, random_strength=1
  D: depth=10, lr=0.03, iter=3000, l2=7, bagging_temperature=1

All 4 configs run sequentially. Best by CV RMSE is selected for submission.

Usage:
    python scripts/run_r05_hpo.py
"""
...
SUBMISSION_PATH = OUTPUT_DIR / "submission_r05_hpo.csv"
...
# Base CatBoost params shared across all configs
...
def train_catboost_config(
...
) -> tuple[CatBoostRegressor, float, int, float]:
    """
    Train CatBoost with a specific hyperparameter config.

    Returns:
        (model, val_rmse, best_iteration, elapsed_seconds)
    """
    params = {**BASE_CATBOOST_PARAMS, **config["params"]}

    log(f"--- Config {config_name}: {config['label']} ---")
    log(f"  Params: depth={params['depth']}, lr={params['learning_rate']}, "
        f"iter={params['iterations']}, l2={params['l2_leaf_reg']}")
    if "random_strength" in params:
        log(f"  random_strength={params['random_strength']}")
    if "bagging_temperature" in params:
        log(f"  bagging_temperature={params['bagging_temperature']}")
    ...
    log(f"  Best val RMSE:  {best_score:.4f}")
    log(f"  Train time:     {elapsed:.1f}s")

    return model, best_score, best_iter, elapsed
...
def main() -> float:
    log("=" * 70)
    log("R05 Hyperparameter Optimization: R03 features + CatBoost HPO")
    log("=" * 70)
    log(f"Configs to try: {list(HPO_CONFIGS.keys())}")
    for name, cfg in HPO_CONFIGS.items():
        log(f"  {name} ({cfg['label']}): {cfg['params']}")
    log("=" * 70)
    ...
    log(f"Feature columns: {len(feature_cols)} total")
    ...
    # ================================================================
    # HPO: Run all configs sequentially
    # ================================================================
    log("=" * 70)
    log("Starting hyperparameter grid search (4 configs)")
    log("=" * 70)

    results: dict[str, dict] = {}

    for config_name, config in HPO_CONFIGS.items():
        model, val_rmse, best_iter, elapsed = train_catboost_config(
            config_name, config,
    ...
    # ================================================================
    log("=" * 70)
    log("HPO Results Comparison")
    log("=" * 70)
    header = f"{'Config':<8} {'Label':<20} {'Val RMSE':>10} {'Best Iter':>10} {'Time (s)':>10}"
    log(header)
    log("-" * len(header))
    for name, res in results.items():
        log(f"{name:<8} {res['label']:<20} {res['val_rmse']:>10.4f} "
            f"{res['best_iter']:>10d} {res['elapsed']:>10.1f}")
    ...
    log("=" * 70)
    log(f"BEST CONFIG: {best_name} ({best_result['label']})")
    log(f"  Val RMSE:  {best_result['val_rmse']:.4f}")
    log(f"  Best iter: {best_result['best_iter']}")
    log(f"  Time:      {best_result['elapsed']:.1f}s")
    log("=" * 70)

    # ================================================================
    # Feature importance (best model)
    # ================================================================
    show_feature_importance(best_model, feature_cols)

    # ================================================================
    # Predict on test with best model
    # ================================================================
    log("Predicting on test set with best config...")
    ...
    log("=" * 70)
    log(f"Best config:     {best_name} ({best_result['label']})")
    log(f"Best val RMSE:   {best_result['val_rmse']:.4f}")
    log(f"Best iteration:  {best_result['best_iter']}")
    log(f"Total features:  {len(feature_cols)} (same as R03)")
    log(f"Total HPO time:  {elapsed_total:.1f}s")
    log(f"  R03 baseline:  17.17 (depth=8, lr=0.1, iter=1000)")
    log("")
    log("Config comparison:")
    for name, res in results.items():
        marker = " <-- BEST" if name == best_name else ""
        log(f"  {name} ({res['label']:<20}): RMSE={res['val_rmse']:.4f}, "
            f"iter={res['best_iter']}, time={res['elapsed']:.0f}s{marker}")
    log("")
