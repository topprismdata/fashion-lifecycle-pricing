"""
Walmart Store Sales Forecasting — R04 Multi-Model Ensemble
No lag features (proven harmful). Focus on:
1. Multi-model: LightGBM + XGBoost + CatBoost
2. Better store-dept aggregated features
3. Holiday-specific feature engineering
4. Per-store-type models
"""
import pandas as pd
import numpy as np
...
# ============================================================
print("=" * 60)
print("R04: Walmart — Multi-Model Ensemble (No Lag)")
print("=" * 60)
...
# ============================================================
# 2. Feature Engineering (no lag, no data leakage)
# ============================================================
def create_features(df, train_ref=None):
    ...
    # Aggregated stats (from train_ref only)
    if train_ref is not None:
        ...
        # Week-of-year avg (seasonal baseline)
        ...
        train_ref_copy['Type_enc'] = train_ref_copy['Type'].map({'A': 0, 'B': 1, 'C': 2})
        type_week_avg = train_ref_copy.groupby(['Type_enc', 'Week'])['Weekly_Sales'].mean().reset_index()
        type_week_avg.columns = ['Type_enc', 'Week', 'type_week_avg']
        df = df.merge(type_week_avg, on=['Type_enc', 'Week'], how='left')

        # Holiday week avg sales
        ...
        # Size-Week interaction
        size_week = train_ref_copy.groupby(['Size', 'Week'])['Weekly_Sales'].mean().reset_index()
        size_week.columns = ['Size', 'Week', 'size_week_avg']
        df = df.merge(size_week, on=['Size', 'Week'], how='left')
...
print(f"  LightGBM val WMAE: {wmae(val['Weekly_Sales'].values, lgb_val_pred, val['IsHoliday'].values):.4f}")
...
print(f"  XGBoost val WMAE: {wmae(val['Weekly_Sales'].values, xgb_val_pred, val['IsHoliday'].values):.4f}")
...
print(f"Best blend weight: LGB={best_w:.1f}, XGB={1-best_w:.1f}")
...
sub_path = OUTPUTS / "submission_r04_ensemble.csv"
